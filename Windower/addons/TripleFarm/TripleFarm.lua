...
-- Enhanced error handling and menu validation
local function validateZoneAccess(zone_name)
	-- Check if player has access to target zones
	local required_zones = {
		["Escha - Zi'Tah"] = "Adoulin mission progress required",
		["Escha - Ru'Aun"] = "Adoulin mission progress required", 
		["Reisenjima"] = "Rhapsodies mission progress required"
	}
	
	if required_zones[zone_name] then
		log('INFO: Ensure you have completed: ' .. required_zones[zone_name])
	end
	return true
end
...
windower.register_event('incoming chunk',function(id,data,modified,injected,blocked)
	...
	if id == 0x034 or id == 0x032 then
		if busy == true then
			...
			-- Handle NPC interaction for elvorseal with better error handling
			if not isBuffActive(BUFFS.ELVORSEAL) then
				log('Getting elvorseal from ' .. npc.name .. '...')
			...
			-- More robust option selection based on boss and zone
			local option_index
			if current_boss == "QUETZ" then
				option_index = 0  -- Confirmed working for Quetz
			elseif current_boss == "AZI" then
				option_index = 11  -- Try standard Escha option
			elseif current_boss == "NAGA" then
				option_index = 11  -- Try standard Escha option
			else
				log('ERROR: Unknown boss configuration!')
				return
			end
			
			log('Attempting to enter ' .. boss_config.target_zone .. ' (option: ' .. option_index .. ')')
			local elvorseal = packets.new('outgoing', 0x05B, {
				["Target"] = npc.id,
...
windower.register_event('addon command', function(...)
	...
	elseif command[1] == 'debug' then
		...
		for id, mob in pairs(mobs) do
			if mob.name and (mob.name == boss_config.npc_name or string.find(mob.name:lower(), 'portal') or string.find(mob.name:lower(), 'confluence')) then
				log('Found NPC: ' .. mob.name .. ' (ID: ' .. id .. ', Index: ' .. mob.index .. ')')
			end
		end
	else
		...
		log('//tf debug - Show debug information')
	end
end) 
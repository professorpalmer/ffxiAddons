...
-- Treasure Hunter automation state (first-hit tagging)
local thState = {
    enabled = false,  -- follows settings.THMode
    pending = false,  -- true when TH set should overlay for the current target
    targetId = nil,   -- server id of the target we need to tag
    overlayed = false,
    lastTp = 0,
};

local function ResetThState()
    thState.pending = false;
    thState.targetId = nil;
    thState.overlayed = false;
end

local function SetThEnabled(enabled)
    settings.THMode = enabled;
...
-- Clear TH on first TP gain while engaged (melee swing / WS landed)
local function MaybeClearThOnTpGain(player)
...
local function ClearThOnAction(reason)
    if (not (thState.enabled and thState.pending and thState.targetId)) then
        return;
    end
    local tgtId = GetTargetId();
...
local function UpdateThTarget(player)
    ...
    if (tgtId ~= nil) then
        if (tgtId ~= thState.targetId) then
            ...
            thState.pending = true; -- new target: apply TH until first landed hit
            thState.overlayed = false;
...
-- Clear TH overlay after the first landed action from the player on the tracked target
...
local sets = {
    ...
    -- Movement speed set (auto-used while moving out of combat)
    Movement = Equip.NewSet {
        Feet = "Hermes' Sandals +1",
    },

    -- Adoulin-only movement body for faster town travel
...
local function EnsureAnimatorEquipped()
    if (not pupLib) then
        return;
    end

    if (thState.pending) then
    ...
    local equippedAnimator = GetEquippedItemName(RANGE_SLOT);
...
profile.OnLoad = function()
    ...
    SetThEnabled(settings.THMode); -- keep TH automation state in sync on load
...
profile.HandleCommand = function(args)
    ...
    elseif (args[1] == 'th') then
        local newState = not thState.enabled;
        SetThEnabled(newState);
        print('Treasure Hunter auto-first-hit: ' .. (newState and 'On' or 'Off'));
        profile.HandleDefault(); -- Refresh gear to apply/remove pending overlay
        return;
...
profile.HandleDefault = function()
    ...
    -- Track TH automation target / pending state when engaged
    UpdateThTarget(player);
    ...
    -- Overlay Treasure Hunter only until first landed hit on current target
    if (thState.pending and sets.TH) then
    ...
    -- Fallback: clear pending on first TP gain if packet detection misses
    MaybeClearThOnTpGain(player);

    -- Overlay movement speed boots when moving outside combat
    if (sets.Movement and player ~= nil and player.Status ~= 'Engaged' and player.IsMoving == true) then
        Equip.Set(sets.Movement);
    end

    -- Wear Councilor's Garb to gain Adoulin movement speed bonus
    if (inAdoulin and sets.Adoulin) then
        Equip.Set(sets.Adoulin);
    end

    -- Ensure appropriate animator is equipped when TH overlay is not active
    EnsureAnimatorEquipped();
end

profile.HandleAbility = function()
    ClearThOnAction('ability');

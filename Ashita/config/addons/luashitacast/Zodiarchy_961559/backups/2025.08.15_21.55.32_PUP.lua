...
local settings = {
    ...
    NakedMode = false,      -- Naked mode for sortie runs
}
...
local function GetBaseSet()
    ...
    -- Check for naked mode (highest priority)
    if (settings.NakedMode) then
        return sets.Naked;
    end
...
profile.HandleDefault = function()
    ...
    -- Skip automatic gear changes if naked mode is active
    if (settings.NakedMode) then
        return;
    end

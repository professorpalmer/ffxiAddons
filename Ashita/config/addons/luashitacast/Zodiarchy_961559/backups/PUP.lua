...
local sets = {
    ...
    Activate = Equip.NewSet {
        Main = "", -- Force a different weapon to test
        Back = petCape,
        Ear2 = KaragozEar,
        Ring1 = "", -- Make it more obvious
    },
...
-- Hotkey handler for DEL key to use Vile Elixir items
profile.HandleHotkey = function(key)
    if (key == 'delete') then
        -- Try to use Vile Elixir+1 first, then Vile Elixir if +1 is not available
        local player = gData.GetPlayer();
        local inventory = gData.GetInventory();
        
        -- Check if Vile Elixir+1 is available
        local hasVileElixirPlus1 = false;
        for i = 1, 80 do
            if (inventory[i] and inventory[i].Name == "Vile Elixir +1") then
                hasVileElixirPlus1 = true;
                break;
            end
        end
        
        if hasVileElixirPlus1 then
            print('Using Vile Elixir +1');
            AshitaCore:GetChatManager():QueueCommand(1, '/item "Vile Elixir +1" <me>');
        else
            -- Check if regular Vile Elixir is available
            local hasVileElixir = false;
            for i = 1, 80 do
                if (inventory[i] and inventory[i].Name == "Vile Elixir") then
                    hasVileElixir = true;
                    break;
                end
            end
            
            if hasVileElixir then
                print('Using Vile Elixir');
                AshitaCore:GetChatManager():QueueCommand(1, '/item "Vile Elixir" <me>');
            else

...
local kotoba = {
    ...
    heartbeat_file = AshitaCore:GetInstallPath() .. '\\addons\\kotoba\\heartbeat.txt',
    ...
    heartbeat_timer = 0,
    heartbeat_interval = 300, -- Touch heartbeat every ~5 seconds (300 frames at 60 FPS)
...
ashita.events.register('d3d_present', 'kotoba_present', function ()
    ...
    -- Heartbeat: touch file so translator knows game is still alive
    kotoba.heartbeat_timer = kotoba.heartbeat_timer + 1;
    if kotoba.heartbeat_timer >= kotoba.heartbeat_interval then
        kotoba.heartbeat_timer = 0;

...
function RollingStrategy.new(dependencies)
	local self = {
		...
		partyAlertTime = 0,
		-- Bust recovery messaging
		bustWaitingMessageSent = false,
	}
...
function RollingStrategy:handleBustRecovery()
	...
	if not self.hasBuff("Bust") then
		-- Reset the waiting message flag when bust is gone
		self.bustWaitingMessageSent = false
		return false
	end
...
function RollingStrategy:determineNextRoll()
	...
	if not haveRoll1 then
		...
		if self.hasBuff('Bust') and self.settings.rollwithbust and self.mainjob == 17 then
			...
			local crookedCD = self.recasts[96] or 0
...
function RollingStrategy:executeRollStrategy(finishRoll)
	...
	-- Simplified Random Deal logic - only use when beneficial
	local function checkAndUseRandomDeal()
		if not self.settings.randomdeal or not self.recasts[196] or self.recasts[196] > 0 then
			return false
		end
		...
		if self.settings.oldrandomdeal then
			...
			if self.hasSnakeEye and self.recasts[197] and self.recasts[197] > 0 then
				table.insert(resetReasons, "Snake Eye")
				shouldUseRandomDeal = true
			end
			if self.hasFold and self.recasts[198] and self.recasts[198] > 0 then
				table.insert(resetReasons, "Fold")
				shouldUseRandomDeal = true
			end
		else
			-- New mode: Reset Crooked Cards primarily, Snake Eye and Fold secondarily
			if self.recasts[96] and self.recasts[96] > 0 then
				table.insert(resetReasons, "Crooked Cards")
				shouldUseRandomDeal = true
			end
			if self.hasSnakeEye and self.recasts[197] and self.recasts[197] > 0 then
				table.insert(resetReasons, "Snake Eye")
